...
void MainMenu::insertNewProject(){
    ...
    if(indicePrimarioL.isEmpty()){
        ...
        counter += newProject.toFile().size() + NEWLINE_SIZE; //Almacena los 18 bytes de todos los caracteres mas un byte del salto de linea
    }
    string asd("\n");
    cout<<"Long salto; "<<asd.size()<<endl;
    cout<<newProject.toFile()<<endl;
    cout<<newProject.toFile().size()<<endl;
...
string MainMenu::readByPrimaryIndex(const CIterator& auxIt,fstream& lecturaIndice){
    ...
    cout<<((IndicePrimario*)(it.readContent()))->getDireccionBytes()<<endl;
    //indicePrimarioL.sortList(true);
    indicePrimarioL.print();
    //cout<<((IndicePrimario*)(it.readContent()))->getDireccionBytes()<<endl;
